...
// tlsConfig.
func NewUpgradeRoundTripper(tlsConfig *tls.Config, followRedirects, requireSameHostRedirects bool, headerUpgrade string) *UpgradeRoundTripper {
	return NewUpgradeRoundTripperWithConfig(RoundTripperConfig{
...
// func NewUpgradeRoundTripperProxy(tlsConfig *tls.Config, followRedirects, requireSameHostRedirects bool, proxier func(*http.Request) (*url.URL, error)) *UpgradeRoundTripper {
// 	return NewUpgradeRoundTripperWithConfig(RoundTripperConfig{
// 		TLS:                      tlsConfig,
// 		FollowRedirects:          followRedirects,
// 		RequireSameHostRedirects: requireSameHostRedirects,
...
// configuration.
func NewUpgradeRoundTripperWithConfig(cfg RoundTripperConfig) *UpgradeRoundTripper {
	if cfg.Proxier == nil {
		cfg.Proxier = utilnet.NewProxierWithNoProxyCIDR(http.ProxyFromEnvironment)
	}
	return &UpgradeRoundTripper{
		tlsConfig:                cfg.TLS,
...
func (s *UpgradeRoundTripper) TLSClientConfig() *tls.Config {
	return s.tlsConfig
}

// Dial implements k8s.io/apimachinery/pkg/util/net.Dialer.
func (s *UpgradeRoundTripper) Dial(req *http.Request) (net.Conn, error) {
	conn, err := s.dial(req)
	if err != nil {
		return nil, err
	}
...
func (s *UpgradeRoundTripper) dialWithoutProxy(ctx context.Context, url *url.URL) (net.Conn, error) {
...
func (s *UpgradeRoundTripper) proxyAuth(proxyURL *url.URL) string {
...
// connection.
func (s *UpgradeRoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	header := req.Header.Clone() //utilnet.CloneHeader(req.Header)
	header.Add(HeaderConnection, HeaderUpgrade)
	header.Add(HeaderUpgrade, s.HeaderUpgrade)
...
func (s *UpgradeRoundTripper) NewConnection(resp *http.Response) (net.Conn, error) {
	connectionHeader := strings.ToLower(resp.Header.Get(HeaderConnection))
	upgradeHeader := strings.ToLower(resp.Header.Get(HeaderUpgrade))
	if (resp.StatusCode != http.StatusSwitchingProtocols) || !strings.Contains(connectionHeader, strings.ToLower(HeaderUpgrade)) || !strings.Contains(upgradeHeader, strings.ToLower(s.HeaderUpgrade)) {
